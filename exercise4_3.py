...
def crossover(parent1, best, parent2, sbest):
    ...
    for i in range(2, N):
        ...
        for j in range(len(input_list)):
            ...
            if j == bit_to_change1:
                signal.value = abs(1-signal.value)
...
def plot_scores(time):
    ...
    axis[time].plot(generation_score)
    axis[time].set_xlabel("Generation")
...
if __name__ == '__main__':
    ...
    for times in range(3):
        plot_scores(times)
    axis[1].set_ylabel("Number of switches")
    axis[0].set_title("Score for 3 executions")
    plt.show()
